...
func ShenghuoServiceParseAndWriteCsv() {
	if len(channels.ShenghuoServiceHtmlNeedParseChan) == 0 {
...
func ShenghuoServiceParsePc(html string) (models.HtmlExtractorResult, error) {
...
func ShenghuoServiceParseMobile(html string) (models.HtmlExtractorResult, error) {
	res := models.HtmlExtractorResult{}
	dom, err := goquery.NewDocumentFromReader(strings.NewReader(html))
	if err != nil {
		return res, err
